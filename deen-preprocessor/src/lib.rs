...
pub enum PreProcessorError {
    ...
    #[diagnostic(severity(Error), code(deen::pre::io_error))]
    Io,

    #[error("Preprocessor module returned syntax error")]
    #[diagnostic(severity(Error), code(deen::pre::syntax_error))]
    Syntax {
        message: String,
...
impl PreProcessor {
    ...
    pub fn process(
        ...
        minipre::process_str(&code, &mut self.context).map_err(|err| match err {
...
impl PreProcessor {
    fn define_os(&mut self) {
        self.context.define(
            "__OS_LINUX__",
            format!("{}", cfg!(target_os = "linux") as u8),
        );
        self.context.define(
            "__OS_WINDOWS__",
            format!("{}", cfg!(target_os = "windows") as u8),
        );
        self.context.define(
            "__OS_MACOS__",
            format!("{}", cfg!(target_os = "macos") as u8),
        );
        self.context.define(
            "__OS_FREEBSD__",
            format!("{}", cfg!(target_os = "freebsd") as u8),
        );
        self.context.define(
            "__OS_NETBSD__",
            format!("{}", cfg!(target_os = "netbsd") as u8),
        );
        self.context.define(
            "__OS_OPENBSD__",
            format!("{}", cfg!(target_os = "openbsd") as u8),
        );
        self.context.define(
            "__OS_ANDROID__",
            format!("{}", cfg!(target_os = "android") as u8),
        );
        self.context
            .define("__OS_IOS__", format!("{}", cfg!(target_os = "ios") as u8));
    ...
    fn define_arch(&mut self) {
        self.context.define(
            "__ARCH_X86__",
            format!("{}", cfg!(target_arch = "x86") as u8),
        );
        self.context.define(
            "__ARCH_X86_64__",
            format!("{}", cfg!(target_arch = "x86_64") as u8),
        );
        self.context.define(
            "__ARCH_ARM__",
            format!("{}", cfg!(target_arch = "arm") as u8),
        );
        self.context.define(
            "__ARCH_AARCH64__",
            format!("{}", cfg!(target_arch = "aarch64") as u8),
        );
        self.context.define(
            "__ARCH_RISCV64__",
            format!("{}", cfg!(target_arch = "riscv64") as u8),
        );
        self.context.define(
            "__ARCH_WASM32__",
            format!("{}", cfg!(target_arch = "wasm32") as u8),
        );
    ...
    fn define_env(&mut self) {
        self.context
            .define("__ENV_GNU__", format!("{}", cfg!(target_env = "gnu") as u8));
        self.context.define(
            "__ENV_MUSL__",
            format!("{}", cfg!(target_env = "musl") as u8),
        );
        self.context.define(
            "__ENV_MSVC__",
            format!("{}", cfg!(target_env = "msvc") as u8),
        );
    }
}
