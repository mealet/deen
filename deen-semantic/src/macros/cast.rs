...
impl MacroObject for CastMacro {
    fn verify_call(&self, analyzer: &mut Analyzer, arguments: &Vec<Expressions>, span: &(usize, usize)) -> Type {
        ...
        if matches!(arguments.first(), Some(Expressions::Argument { name: _, r#type: _, span: _ }))
        || !matches!(arguments.iter().nth(1), Some(Expressions::Argument { name: _, r#type: _, span: _ })) {
            analyzer.error(
                format!("Wrong macro call! Usage: cast!(EXPRESSION, TYPE)"),
                *span
            );
        }

        let (from_type, target_type) = (analyzer.visit_expression(&arguments[0], None), analyzer.visit_expression(&arguments[1], None));
        analyzer.verify_cast(&from_type, &target_type).unwrap_or_else(|err| {
